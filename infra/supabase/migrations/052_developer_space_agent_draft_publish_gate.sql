-- ============================================================
-- Developer Space agent owner-confirmed draft publish gate
-- ============================================================
...
alter table public.developer_space_agent_execution_receipts
  ...
  check (action in ('request_capability', 'save_project_update_draft', 'publish_to_page'));
...
create policy "developer_space_agent_execution_receipts_all_owner"
  ...
  for all using (
    ...
    and exists (
      ...
      and c.action in ('request_capability', 'save_project_update_draft', 'publish_to_page')
...
comment on table public.developer_space_agent_execution_receipts is
  'Owner-scoped Developer Agent receipts. request_capability remains inert planning evidence; save_project_update_draft saves a private owner-only draft; publish_to_page records a bounded owner-confirmed draft publication.';

comment on column public.developer_space_agent_execution_receipts.receipt_payload is
  'Sanitized route-generated receipt facts. Do not store document bodies, route-only ids, prompts, raw payloads, provider data, keys, signing material, logs, cookies, tokens, environment values, private owner content, confirmation ids, owner ids, or preview hashes.';

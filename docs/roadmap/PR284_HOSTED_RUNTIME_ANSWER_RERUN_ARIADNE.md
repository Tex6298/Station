...
Verify PR283 on hosted Railway after deployment: replay the bounded runtime
chat/context path and decide whether the full two-anchor recall bar is now live.

PR283 added a prompt-only final answer-focus guard after the stable voice close.
The guard makes the latest owner message plus selected-context answer focus
outrank prior chat history, earlier assistant guesses, or persona flourish.

PR284 is product evidence, not another patch lane.
...
- The hosted deployment includes PR283 implementation commit `8783a02b` or a
  later `main` commit.

If Railway has not deployed the PR283 implementation yet, wait and retry. If it
...
2. Intended replay persona.
   - Use the same intended private replay persona from PR282.
   - If the persona selection is ambiguous, report the ambiguity to MIMIR.
3. Bounded chat/context path.
   - Reuse the PR274/PR282 synthetic staging prompt shape.
...
Do not patch product code in PR284.
...
If hosted evidence fails after the PR283 deploy is fresh, wake MIMIR with the
...
Codename: MIMIR
Summary:
- ARIADNE completed PR284 hosted runtime answer rerun.
...
Recommendation:
- [close PR283 hosted recall bar / open repair lane / wait for deploy / other exact next owner]
```

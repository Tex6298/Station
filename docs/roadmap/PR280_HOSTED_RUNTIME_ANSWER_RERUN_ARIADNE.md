...
Verify PR279 on hosted Railway after deployment: replay the bounded runtime
chat/context path and decide whether the full two-anchor recall bar is now live.

PR279 repaired the hosted/local mismatch found by PR278. Retrieval could select
the exact active Memory row, but private runtime context assembly used
`summary ?? content`, so a partial Memory summary could discard full Memory
content before prompt assembly. Selected private Memory now contributes content
plus a differing summary before existing topology trimming.

PR280 is product evidence, not another patch lane.
...
- The hosted deployment includes PR279 implementation commit `7ab41536` or a
  later `main` commit.

If Railway has not deployed the PR279 implementation yet, wait and retry. If it
...
2. Intended replay persona.
   - Use the same intended private replay persona from PR278.
   - If the persona selection is ambiguous, report the ambiguity to MIMIR.
3. Bounded chat/context path.
   - Reuse the PR274/PR278 synthetic staging prompt shape.
...
Do not patch product code in PR280.
...
If hosted evidence fails after the PR279 deploy is fresh, wake MIMIR with the
...
Codename: MIMIR
Summary:
- ARIADNE completed PR280 hosted runtime answer rerun.
...
Recommendation:
- [close PR279 hosted recall bar / open repair lane / wait for deploy / other exact next owner]
```

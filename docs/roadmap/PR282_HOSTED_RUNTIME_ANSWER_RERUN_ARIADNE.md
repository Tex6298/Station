# PR282 - Hosted Runtime Answer Rerun After Grounding Repair
...
Verify PR281 on hosted Railway after deployment: replay the bounded runtime
chat/context path and decide whether the full two-anchor recall bar is now live.

PR281 repaired the answer-behavior layer found by PR280. Private persona prompts
with selected context now include a grounded-answer rule for direct factual
questions, preserving safe requested answer shapes while avoiding long source
copying.

PR282 is product evidence, not another patch lane.
...
- The hosted deployment includes PR281 implementation commit `4c96bbd4` or a
  later `main` commit.

If Railway has not deployed the PR281 implementation yet, wait and retry. If it
...
2. Intended replay persona.
   - Use the same intended private replay persona from PR280.
   - If the persona selection is ambiguous, report the ambiguity to MIMIR.
3. Bounded chat/context path.
   - Reuse the PR274/PR280 synthetic staging prompt shape.
...
Do not patch product code in PR282.
...
If hosted evidence fails after the PR281 deploy is fresh, wake MIMIR with the
...
Codename: MIMIR
Summary:
- ARIADNE completed PR282 hosted runtime answer rerun.
...
Recommendation:
- [close PR281 hosted recall bar / open repair lane / wait for deploy / other exact next owner]
```

...
Verify PR285 on hosted Railway after deployment: replay the bounded runtime
chat/context path and decide whether the full two-anchor recall bar is now live.

PR285 repaired private Memory prompt assembly so selected Memory titles or
labels reach the provider-facing private prompt alongside selected Memory
content. PR284 already proved hosted context was full and answer behavior had
improved enough to recall both invented phrases; PR286 must prove the paired
accepted concept labels are also preserved in the answer.

PR286 is product evidence, not another patch lane.
...
- The hosted deployment includes PR285 implementation commit `2d37b1e9` or a
  later `main` commit.

If Railway has not deployed the PR285 implementation yet, wait and retry. If it
...
2. Intended replay persona.
   - Use the same intended private replay persona from PR284.
   - If the persona selection is ambiguous, report the ambiguity to MIMIR.
3. Bounded chat/context path.
   - Reuse the PR274/PR282/PR284 synthetic staging prompt shape.
...
Do not patch product code in PR286.
...
If hosted evidence fails after the PR285 deploy is fresh, wake MIMIR with the
...
Codename: MIMIR
Summary:
- ARIADNE completed PR286 hosted runtime answer rerun.

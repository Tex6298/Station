...
| NVIDIA platform chat | `packages/ai/src/providers/router.ts`, API env `NVIDIA_AI_API_KEY`, health readiness provider checks, provider-router tests, and PR433 synthetic proof. | Uses OpenAI-compatible chat provider path; does not change embeddings. | Staging-proven synthetic platform chat option, no schema migration. Current `openai/gpt-oss-120b` label is callable. | Exact-output compliance was noisy; trial/data-policy and sensitive replay expectations still need MIMIR/Marty decision before private context is sent. | Hybrid supplement for chat. |

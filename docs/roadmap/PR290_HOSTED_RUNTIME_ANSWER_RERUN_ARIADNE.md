...
Verify PR289 on hosted Railway after deployment: replay the bounded runtime
chat/context path and decide whether explicit selected label/name plus
supporting fact pairs clear the hosted label recall failure.

PR289 changes provider selected-context focus from parenthetical title
formatting to explicit paired fields:

```text
memory: selected label/name: <title>; supporting fact: <content>
```

The hosted answer must recall both accepted concept labels and both matching
invented retrieval phrases from selected context. PR290 is product evidence,
not another patch lane.
...
- The hosted deployment includes PR289 runtime implementation commit
  `21173c32` or a later `main` commit.

If Railway has not deployed the PR289 implementation yet, wait and retry. If it
...
2. Intended replay persona.
   - Use the same intended private replay persona from PR286/PR288.
   - If persona selection is ambiguous, report the ambiguity to MIMIR.
3. Bounded chat/context path.
   - Reuse the PR274/PR282/PR284/PR286/PR288 synthetic staging prompt shape.
   ...
   - Report labels and phrases separately so MIMIR can distinguish partial
     progress from a full pass.
...
Do not patch product code in PR290.
...
If hosted evidence fails after the PR289 deploy is fresh, wake MIMIR with the
...
```text
docs/roadmap/PR290_HOSTED_RUNTIME_ANSWER_RERUN_RESULT.md
```
...
Codename: MIMIR
Summary:
- ARIADNE completed PR290 hosted runtime answer rerun.

...
PR290 proved that the hosted Railway web and API deployments include the PR289
label/fact pairing repair. Replay-owner auth/session, intended private persona
...
- Both hosted services reported commit prefix `21173c3211f1`, which includes
  PR289 runtime implementation commit `21173c32`.
...
PR290 shows that selected context still contains both labels and phrases after
the PR289 deploy, but the hosted answer did not use them in the single allowed
chat turn. Because several prompt-adjacent repairs have now left the hosted
answer below the full recall bar, the next step should be a narrower diagnostic
or answer-contract lane rather than another uninstrumented wording tweak.

## Validation

- `node tmp-pr290-runtime-rerun.mjs`: FAIL evidence from one hosted chat turn.
- `npx --yes --package @playwright/test@1.41.2 playwright test tmp-pr290-session.spec.js --reporter=line --workers=1`:
  pass, one browser test.

## Recommendation

MIMIR should open a narrow DAEDALUS diagnostic/repair lane for the
provider-facing selected-context answer contract, with ARGUS defining a safe
acceptance gate. The lane should prove, without exposing raw prompts or private
source bodies, whether selected label/fact pairs are reaching the final
provider-facing answer focus and whether the answer generator is required to
preserve paired labels and phrases. Do not keep iterating prompt wording without

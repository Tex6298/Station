...
2. Chat `_debug` payloads are now production-gated diagnostics:
   ...
   - allowed in production only for admin/operator requests when `STATION_EXPOSE_AI_DEBUG=true`.
...
6. Repeat in production mode with an admin request and `STATION_EXPOSE_AI_DEBUG=true`; confirm `_debug` appears only for that admin/operator path.

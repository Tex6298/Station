...
a clean non-production hosted Checkout proof. PR432 later proves the current
bounded `station_free_1536` staging replay corpus with migration `029`,
Gemini/1536/backfill-v2 rows, and hostile read-only retrieval smoke. The dirty
replay owner remains dirty and untouched.
...
DAEDALUS PR432 proof on 2026-06-28: the migration-029 blocker and current
bounded corpus retrieval proof are resolved for staging. See
...
- Future embedding corpus/provider/model/dimension changes need a fresh scoped
  migration/reindex/smoke proof. The current bounded staging replay corpus is
  covered by PR432.

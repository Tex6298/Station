...
function app(){const value=express();value.use(express.json());value.use(institutionPublicationsRouter);value.get("/unrelated-public",(_req,res)=>res.json({public:true}));return value}
...
test("member draft, optimistic edit, owner publication, retraction, and public boundary stay exact",async()=>{const db=new Fake();setSupabaseAdminForTests(db.client as any);const value=app();try{
  assert.deepEqual((await request(value,"GET","/unrelated-public")).body,{public:true});
  ...
  db.tables.projects[0].visibility="private";const hiddenDetail=await request(value,"GET","/institutions/station-institutional-alpha/publications/field-note","owner-token");assert.equal(hiddenDetail.body.publication.publicHref,null);assert.equal((await request(value,"GET","/institutions/station-institutional-alpha/publications/public/field-note")).status,404);db.tables.projects[0].visibility="public";const restoredDetail=await request(value,"GET","/institutions/station-institutional-alpha/publications/field-note","owner-token");assert.match(restoredDetail.body.publication.publicHref,/\/publications\/public\/field-note$/);assert.equal((await request(value,"GET","/institutions/station-institutional-alpha/publications/public/field-note")).status,200);

...
test("migration 083 owns visible thread reply counts transactionally", () => {
  ...
  assert.match(sql, /comments_table_owner name/);
  assert.match(sql, /threads_table_owner name/);
  assert.match(sql, /comments_table_owner is distinct from threads_table_owner/);
  assert.match(sql, /current_user is distinct from comments_table_owner/);
  ...
  assert.match(sql, /trusted_activity_at timestamptz := null/);
